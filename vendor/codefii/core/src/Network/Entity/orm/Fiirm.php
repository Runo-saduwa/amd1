...
abstract class Fiirm extends Database
{
    ...
    private function fields($data = null)
    {
     ...
     public function __set($name,$value){
        $this->allfields{$name} = $value;
    }
    public function __get($name){
         if (array_key_exists($name, $this->allfields)) {
    ...
    public function save($data=null)
    {
        if(empty($data)){
            ...
            if (array_key_exists($this->pk, $this->allfields)) {
            $this->count = $this->findOne([$this->pk => $this->allfields[$this->pk]]);
            $this->lastId = $this->allfields[$this->pk];
        }

        if ( ! empty($this->count)) {
            return $this->update($this->allfields);
        }
         $this->isAdded = true;

        return $this->create($this->allfields);
        }else{
